package com.htl.hr.det 
import java.text.SimpleDateFormat
import java.text.DateFormat
import static org.springframework.http.HttpStatus.*
import grails.transaction.Transactional
import grails.converters.JSON
import com.htl.cmn.det.*
import hashencode.HashEncode
import com.htl.cmn.crypto.*
import com.htl.gen.det.*
 
@Transactional(readOnly = true)
class HrEmpMstrController {
    
    static allowedMethods = [save: "POST", update: "POST", delete: "DELETE"]

    
    def create() { }
    def index()  { }
    
    @Transactional
    def save(HrEmpMstr hrEmpInstance){

        CmnUsrMastr userInstance=new CmnUsrMastr()
        
        userInstance.statsFlg = 'A';
        userInstance.delFlg = 'N';
        userInstance.cretnBy = request.getCookie('htl_Pchrt_sd').toInteger();            
        userInstance.cretnDate = new Date();
        userInstance.usrCd=params.usrName
        userInstance.usrName=params.empName
        def ob=new HashEncode()           
        userInstance.usrPaswrd=ob.md5Java(params.userPaswrd)
        //objUser.usrType=params.empType
        userInstance.usrType="E"
                 
            
        if(userInstance.save(flush:true)){
            println (userInstance)
            hrEmpInstance.empUsrId=userInstance.id
        }
        else{
            
            if (!userInstance.validate()) {
                if(CmnUsrMastr.findByUsrCd(params.usrName)) flash.message= "User Name should be Unique"
                //flash.message=userInstance.errors
                println ("::"+userInstance.errors)
                //respond hrEmpInstance
                respond hrEmpInstance, view:'create'
                return
            }
            
        }
        try{
            if(!(request.getFile("proPic1").bytes==null||request.getFile("proPic1").bytes==""||request.getFile("proPic1").bytes==[]))
            {
                hrEmpInstance.profPic=request.getFile("proPic1").bytes
                hrEmpInstance.picType = request.getFile("proPic1").contentType   

            }   
            
        }catch(Exception e){
            println e
            flash.message= "Image not inserted!! Try again"
            respond hrEmpInstance, view:'create'
            return
        }
           
        if(params.empTypeId!="null")hrEmpInstance.empType=HrEmpTypeMastr.get(params.empTypeId)
        if(params.desigId!="null")hrEmpInstance.desig=com.htl.gen.det.GenDesigMastr.get(params.desigId)
        if(params.depId!="null")hrEmpInstance.dep=com.htl.gen.det.GenDepMstr.get(params.depId)
        
        hrEmpInstance.delFlg='N'        
        //hrEmpInstance.cretnBy=1
        hrEmpInstance.cretnBy = request.getCookie('htl_Pchrt_sd').toInteger();
        hrEmpInstance.cretnDate= new Date()
        //hrEmpInstance.dob=null
        hrEmpInstance.dob = new  SimpleDateFormat("dd/mm/yyyy", Locale.ENGLISH).parse(params.get("dobStr"));
        
        if(params.get("dojStr")!="")hrEmpInstance.doj = new  SimpleDateFormat("dd/mm/yyyy", Locale.ENGLISH).parse(params.get("dojStr"));
        //        def dt=new  SimpleDateFormat("yyyy-MM-dd", Locale.ENGLISH).parse(params.get("dob"));
        //        println(dt)
        
        //        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss") //required format
        //        hrEmpInstance.dob = sdf.parse(params.dob)

        if (!hrEmpInstance.validate()) {                
            if(HrEmpMstr.findByEmpId(params.empId)) flash.message= "Employee ID should be Unique!"                 
            println ("::"+hrEmpInstance.errors)
            respond hrEmpInstance.errors, view:'create'
            return
        } 
        
        hrEmpInstance.save flush:true
        flash.message= "Created"        
        redirect action:'index'
    
    }
    
    
    
    def GetList()
    {
        

        def varr=HrEmpMstr.createCriteria().list() {                    
            if(params.empId!="" && params.empId!=null) 
            {ilike("empId","${params.empId}%")  }
            if(params.empName!="" && params.empName!=null) 
            ilike("empName","${params.empName}%")
            if(params.mobNo!="" && params.mobNo!=null) 
            ilike("mobNo","${params.mobNo}%")
            //q("dep.id","${params.depId}%")
            if(params.depId!="" && params.depId!=null) {dep{
                    ilike("depName","${params.depId}%")
                }}
            ne("delFlg","Y")
            order("id", "desc")
        }
                            
        def strDep="" 
        //        def varr=GenDepMstr.list()        
        def res = varr.collect{
            strDep="" 
            if(it.depId!=null)
            {
                def obj=GenDepMstr.get(it.depId)
                strDep=obj.depName
            }            
            
            [                    
                id:it.id,
                empId:it.empId,     
                empName:it.empName,                    
                mobNo:it.mobNo,                    
                depId:strDep,     
            ]
        }
        def jsonData=[row:res]
        render jsonData as JSON
    }
    
    def showimage(HrEmpMstr hrEmpInstance){ 
           
        try{
            byte [] image = hrEmpInstance.profPic
            // byte array
            response.setHeader('Content-length', "${image.length}")
            response.contentType = hrEmpInstance.picType // or the appropriate image content type
            response.outputStream << image
            response.outputStream.flush()
        } 
        catch(Exception e){
            String encoded=""
            byte[] decoded = encoded.decodeBase64()
            response.outputStream << decoded            
            response.outputStream.flush()            
        }
    }

    
    def edit(HrEmpMstr hrEmpMstrInstance){   
       
        DateFormat df = new SimpleDateFormat("dd/mm/yyyy")
        if(hrEmpMstrInstance.doj!=null){
        
            hrEmpMstrInstance.dojStr = df.format(hrEmpMstrInstance.doj)
        }
        hrEmpMstrInstance.dobStr = df.format(hrEmpMstrInstance.dob)    
    
    
        def userInstance=CmnUsrMastr.get(hrEmpMstrInstance.empUsrId)
       
        
        //        Date date = Date.parse("yyyy-MM-dd HH:mm:ss[.fraction]",hrEmpInstance.doj)
        //hrEmpInstance.dojstr = hrEmpInstance.dojstr.format( 'yyyy-MM-dd' )
        
        //         hrEmpInstance.dojstr= Date.parse("yyyy-MM-dd hh:mm:ss", hrEmpInstance.dojstr).format("dd/MM/yyyy")
        
        respond hrEmpMstrInstance,model:[userInstance:userInstance]
    }
   
    @Transactional
    def update(HrEmpMstr hrEmpInstance){   
        //println (params)
        def userInstance=CmnUsrMastr.get(hrEmpInstance.empUsrId)
        if(params.del)
        {
            HrEmpMstr.executeUpdate("delete HrEmpMstr where delFlg = 'Y' and empId=?",[params.empId])
            hrEmpInstance.delFlg='Y'   
            userInstance.delFlg='Y'   
            flash.message= "Deleted"
        }
        else{
            if (!hrEmpInstance.validate()) {                
                if(HrEmpMstr.findByEmpId(params.empId)) flash.message= "Employee ID should be Unique!"                 
                println ("::"+hrEmpInstance.errors)
                respond hrEmpInstance.errors, view:'edit'
                return
            }  
            if(params.empTypeId!="null")hrEmpInstance.empType=HrEmpTypeMastr.get(params.empTypeId) else hrEmpInstance.empType=null
            if(params.desigId!="null")hrEmpInstance.desig=com.htl.gen.det.GenDesigMastr.get(params.desigId) else hrEmpInstance.desig=null
            if(params.depId!="null")hrEmpInstance.dep=com.htl.gen.det.GenDepMstr.get(params.depId) else hrEmpInstance.dep=null
        
            
            hrEmpInstance.dob = new  SimpleDateFormat("dd/mm/yyyy", Locale.ENGLISH).parse(params.get("dobStr"));
            if(params.get("dojStr")!="")hrEmpInstance.doj = new  SimpleDateFormat("dd/mm/yyyy", Locale.ENGLISH).parse(params.get("dojStr"));   
            else hrEmpInstance.doj=null
            
            try{
                            
                if(!(request.getFile("proPic1").bytes==null||request.getFile("proPic1").bytes==""||request.getFile("proPic1").bytes==[]))
                {
                    hrEmpInstance.profPic=request.getFile("proPic1").bytes
                    hrEmpInstance.picType = request.getFile("proPic1").contentType                
                } 
            }catch(Exception e){
                println e
                flash.message= "Image not inserted!! Try again"
                DateFormat df = new SimpleDateFormat("dd/mm/yyyy")
                if(hrEmpInstance.doj!=null){
        
                    hrEmpInstance.dojStr = df.format(hrEmpInstance.doj)
                }
                hrEmpInstance.dobStr = df.format(hrEmpInstance.dob)
                respond hrEmpInstance, view:'edit'
                return
            }
            if(params.iDlt=="Y"){
                hrEmpInstance.profPic=null
                hrEmpInstance.picType = null  
            }
            if(userInstance.usrName!=hrEmpInstance.empName) userInstance.usrName=params.empName            
            if(params.userPaswrd!="") userInstance.usrPaswrd=params.userPaswrd
            if(params.userPaswrd!=""||userInstance.usrName!=hrEmpInstance.empName){
                userInstance.updtdBy=request.getCookie('htl_Pchrt_sd').toInteger()
                userInstance.updtdDate= new Date()
              
            }
            
            flash.message= "Updated"
            
        } 
        hrEmpInstance.updtdBy=request.getCookie('htl_Pchrt_sd').toInteger();
        hrEmpInstance.updtdDate= new Date()
        hrEmpInstance.save flush:true
        redirect action:'index'
    }
    
   
}